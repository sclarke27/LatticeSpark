 ...
 *
 * Demonstrates magnetic field detection using the LatticeSpark framework.
 * Uses Hall effect sensor on GPIO pin 12.
 *
 * This example shows:
 * 1. Loading configuration from JSON file
 * 2. Initializing the coordinator
 * 3. Polling for magnet presence
 * 4. Detecting state changes
 * 5. Proper cleanup
 *
 * Usage:
 *   node examples/hall-sensor.js
 *
 * Requirements:
 * - Hall effect sensor on GPIO 12
...
async function main() {
  ...
  console.log('LatticeSpark Framework - Hall Effect Sensor Example');
  console.log('='.repeat(60));
  console.log('');
  console.log('Hold a magnet near the sensor to detect it.');
  ...
  try {
    ...
    const initial = await coordinator.read('hall_sensor');
    console.log(`   Magnet detected: ${initial.detected}`);
    console.log('');

    // Step 3: Poll for changes
    console.log('3. Monitoring for magnet changes (10 seconds)...');
    console.log('');
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const reading = await coordinator.read('hall_sensor');

      if (reading.detected !== lastDetected) {
        const timestamp = new Date(reading.timestamp * 1000).toISOString();
        if (reading.detected) {
          console.log(`   [${timestamp}] Magnet detected!`);
        } else {
          console.log(`   [${timestamp}] Magnet removed`);
        }
    ...
    if (error.message.includes('gpiozero')) {
      ...
      console.error('Solution: Check sensor connection on GPIO 12');
    }
